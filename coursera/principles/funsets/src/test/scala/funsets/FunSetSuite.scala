...
class FunSetSuite extends FunSuite {
  ...
  test("difference contains elements that are in first set but not in second set") {
    new TestSets {
  ...
  test("filter selects only the elements of a set that are accepted by a given predicate p") {
    new TestSets {
      var s = filter(union(s1, union(s2, s3)), x => x < 3)
      assert(contains(s, 1), "filter 1")
      assert(contains(s, 2), "filter 2")
      assert(!contains(s, 3), "filter 3")

      s = filter(union(s1, union(s2, s3)), x => x > 2)
      assert(!contains(s, 1), "filter 1")
      assert(!contains(s, 2), "filter 2")
      assert(contains(s, 3), "filter 3")
    }
